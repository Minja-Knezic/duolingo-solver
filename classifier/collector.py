...
def main():
    ...
    while True:
        ...
        if event.event_type == keyboard.KEY_DOWN:
            ...
            elif event.name == match_words_hotkey.lower():
                print("🔍 Match words hotkey pressed.")
                screenshot = pyautogui.screenshot(region=region)
                file_count = len([f for f in os.listdir(f"{save_folder}/match_words") if f.endswith(".png")])
                filename = os.path.join(save_folder, f"match_words/match_words_{file_count}.png") # BE CAREFUL IF YOU EVER DELETE FILES

                screenshot.save(filename)
                time.sleep(0.2)  
                print(f"✅ Screenshot saved: {filename}")
                # Add your logic here
            elif event.name == type_sentence_hotkey.lower():
                print("🔤 Type sentence hotkey pressed.")
                screenshot = pyautogui.screenshot(region=region)
                file_count = len([f for f in os.listdir(f"{save_folder}/type_sentence") if f.endswith(".png")])
                filename = os.path.join(save_folder, f"type_sentence/type_sentence_{file_count}.png")
                
                screenshot.save(filename)
                time.sleep(0.2)  
                print(f"✅ Screenshot saved: {filename}")
                # Add your logic here
            elif event.name == fill_blank_hotkey.lower():
                print("📝 Fill blanks hotkey pressed.")
                screenshot = pyautogui.screenshot(region=region)
                file_count = len([f for f in os.listdir(f"{save_folder}/fill_blank") if f.endswith(".png")])
                filename = os.path.join(save_folder, f"fill_blank/fill_blank_{file_count}.png")

                screenshot.save(filename)
                time.sleep(0.2)  
                print(f"✅ Screenshot saved: {filename}")
                # Add your logic here
            elif event.name == choose_words_hotkey.lower():
                print("🗂️ Choose words hotkey pressed.")
                screenshot = pyautogui.screenshot(region=region)
